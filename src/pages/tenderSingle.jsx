...
class tenderSingle extends Component {
  constructor(props) {
    super(props);
    this.state = { tenders_array: [], loading: true};
  }

  componentDidMount() {
    Axios.get(`http://uz.orikzor.com/tenders`).then(response => {
      this.setState({ tenders_array: response.data.data, loading: false });
    });
  }
  render() {
    let s = this.props.tender.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
    let others = this.props.tender.filter(n => {
      return (
        n.one_id !== this.props.match.params.id && n.lang === this.props.lang
      );
    });

    let single = this.state.tenders_array.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
...
export default connect(mapStateToProps, { tenders })(tenderSingle);
