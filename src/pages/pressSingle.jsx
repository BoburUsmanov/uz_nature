...
class pressSingle extends Component {
  constructor(props) {
    super(props);
    this.state = { releases_array: [], loading: true};
  }

  componentDidMount() {
    Axios.get(`http://uz.orikzor.com/releases`).then(response => {
      this.setState({ releases_array: response.data.data, loading: false });
    });
  }
  render() {
    let s = this.props.release.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
    let others = this.props.release.filter(n => {
      return (
        n.one_id !== this.props.match.params.id && n.lang === this.props.lang
      );
    });

    let single = this.state.releases_array.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
...
export default connect(mapStateToProps, { releases })(pressSingle);
