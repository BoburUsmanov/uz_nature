...
class interviewsSingle extends Component {
  constructor(props) {
    super(props);
    this.state = { interviews_array: [], loading: true};
  }

  componentDidMount() {
    Axios.get(`http://uz.orikzor.com/interviews`).then(response => {
      this.setState({ interviews_array: response.data.data, loading: false });
    });
  }
  render() {
    let s = this.props.interview.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
    let others = this.props.interview.filter(n => {
      return (
        n.one_id !== this.props.match.params.id && n.lang === this.props.lang
      );
    });

    let single = this.state.interviews_array.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
...
export default connect(mapStateToProps, { interviews })(interviewsSingle);
