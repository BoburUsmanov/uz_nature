...
class economicSingle extends Component {
  constructor(props) {
    super(props);
    this.state = { economics_array: [], loading: true};
  }

  componentDidMount() {
    Axios.get(`http://uz.orikzor.com/economics`).then(response => {
      this.setState({ economics_array: response.data.data, loading: false });
    });
  }
  render() {
    let s = this.props.economic.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
    let others = this.props.economic.filter(n => {
      return (
        n.one_id !== this.props.match.params.id && n.lang === this.props.lang
      );
    });

    let single = this.state.economics_array.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
...
export default connect(mapStateToProps, { economics })(economicSingle);
