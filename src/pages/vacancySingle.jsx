...
class vacancySingle extends Component {
  constructor(props) {
    super(props);
    this.state = { vacations_array: [], loading: true};
  }

  componentDidMount() {
    Axios.get(`http://uz.orikzor.com/vacations`).then(response => {
      this.setState({ vacations_array: response.data.data, loading: false });
    });
  }
  render() {
    let s = this.props.vacation.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
    let others = this.props.vacation.filter(n => {
      return (
        n.one_id !== this.props.match.params.id && n.lang === this.props.lang
      );
    });

    let single = this.state.vacations_array.find(n => {
      return (
        n.one_id === this.props.match.params.id && n.lang === this.props.lang
      );
    });
...
export default connect(mapStateToProps, { vacations })(vacancySingle);
